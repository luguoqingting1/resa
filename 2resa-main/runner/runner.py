...
class Runner(object):
    ...
    def to_cuda(self, batch):  # 遍历 batch 中的每个元素，如果元素的键（k）不是 'meta'，则将对应的值移动到 GPU 上。

        for k in batch:
            if k == 'meta':
                continue
            batch[k] = batch[k].cuda()
        return batch    # 返回迁移后的 batch
    
    def train_epoch(self, epoch, train_loader):
        self.net.train()
        end = time.time()
        max_iter = len(train_loader)  # 返回数据加载器中的批次数量，即一个 epoch 中包含的 mini-batch 数量
        for i, data in enumerate(train_loader):
            if self.recorder.step >= self.cfg.total_iter:
                break
            date_time = time.time() - end  # 记录当前批次的数据加载时间
            self.recorder.step += 1  # 记录当前训练步骤
            data = self.to_cuda(data)  # 把数据转移到GPU上，在GPU上训练
            output = self.trainer.forward(self.net, data)
            self.optimizer.zero_grad()  # 将优化器中的梯度清零
            loss = output['loss']
            loss.backward()  # 反向传播，计算梯度
            self.optimizer.step()  # 根据梯度更新模型参数
            self.scheduler.step()   # 调整学习率的调度器，可能在每个 epoch 结束后更新学习率
            ...
            if i % self.cfg.log_interval == 0 or i == max_iter - 1:  # 达到日志间隔或者是最后一个批次
    ...
    def train(self):
        ...
        for epoch in range(self.cfg.epochs):  # epochs =12
