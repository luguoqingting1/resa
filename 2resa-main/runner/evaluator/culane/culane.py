...
def check():
    ...
    result = subprocess.call(
        '/kaggle/working/resa/2resa-main/runner/evaluator/culane/lane_evaluation', stdout=FNULL, stderr=FNULL)
    if result > 1:
...
def call_culane_eval(data_dir, output_path='/kaggle/working/resa/2resa-main/output'):
    ...
    eval_cmd = '/kaggle/working/resa/2resa-main/runner/evaluator/culane/lane_evaluation'
...
class CULane(nn.Module):
    ...
    def evaluate(self, dataset, output, batch):
        ...
        batch_size = seg.size(0)  # 批次大小，包含多少个样本
        img_name = batch['meta']['img_name']
        img_path = batch['meta']['full_img_path']
        for i in range(batch_size):
            coords = dataset.probmap2lane(predictmaps[i], exists[i])  # 第i个样本
